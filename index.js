    ...
    if (data.eventType != undefined) {
        ...
        if (fuser == -144372147 && data.eventType == chk) {
            connection.query("SELECT * FROM `admins` WHERE `peer` = ? AND `status` = 3", [peer], async function (err, ress11, f) {
                if (ress11.length != 0) {
                    cmdsfnc.commands['uploadstatscommand'](data)
        ...
        if ((data.eventType == chk) || (data.eventType == chk2)) {
            if (data.eventType == chk) {
                ...
                stats.query("SELECT * FROM `users` WHERE `peer` = ? AND `userid` = ?", [peer, banned], async function (err, isprofiled, f) {
                    if (isprofiled.length == 0) {
                        let date = moment().unix() + 10800
                        stats.query("INSERT INTO `users` (`peer`, `userid`, `date`, `nickname`) VALUES (?, ?, ?, ?);", [peer, banned, date, 'нет'], function (error, result, fields) {
                        })
                    }
                })
            }
                    ...
                    if (chkwrn.length == 1) {
                        if (chkwrn[0].number == 1) {
                            ...
                            connection.query("UPDATE `warns` SET `number` = ? WHERE `warns`.`id` = ?;", [1, chkwrn[0].id], function (error, result, fields) {
                                let messageto = 'Вам вынесено первое предупреждение, когда их будет 3 Вы будете исключены из чата! Старайтесь не использовать нецензурную лексику и оскорбления при общении друг с другом!' + ' #user' + user_warned
...
vk.updates.hear(/^!статистика/i, data => {
    let cmdchk = 'stats'
    checkrights(data, cmdchk)
})

vk.updates.hear(/^!help/i, data => {
    data.reply('Инструкцию по использованию бота можно найти здесь: https://vk.com/@weeb_bot-usage')
})
...
vk.updates.hear(/^!apply/i, async data => {
    let peer = data.peerId;
    let user = data.senderId;
    vk.api.messages.getConversationsById({peer_ids: peer, access_token: t1ken, v: v}).then(res => {
        if (res.count != 0) {
            connection.query("SELECT * FROM `admins` WHERE `peer` = ? AND `status` = 5", [peer, user], async function (err, ress, f) {
                if (ress.length == 0) {
                    let user_id = res.items[0].chat_settings.owner_id
                    connection.query("INSERT INTO `admins` (`peer`, `userid`, `status`) VALUES (?, ?, ?);", [peer, user_id, 5], async function (error, result, fields) {
                        connection.query("INSERT INTO `settings` (`peer`) VALUES (?);", [peer], async function (error, result, fields) {
                            connection.query("INSERT INTO `prmsg` (`peer`, `enabled`) VALUES (?, ?);", [peer, 1], async function (error, result, fields) {
...
vk.updates.hear(/^!uploadstats/i, data => {
    let cmdchk = 'uploadstats'
    checkrights(data, cmdchk)
})

vk.updates.hear(/^!setnick/i, data => {
    let cmdchk = 'setnick'
    checkrights(data, cmdchk)
});

vk.updates.hear(/^!remnick/i, data => {
    let cmdchk = 'remnick'
    checkrights(data, cmdchk)
});
...
vk.updates.hear(/^!stmoder/i, data => {
    let cmdchk = 'stmoder'
    checkrights(data, cmdchk)
})

vk.updates.hear(/^!unstmoder/i, data => {
    let cmdchk = 'unstmoder'
    checkrights(data, cmdchk)
})
    ...
    connection.query("SELECT * FROM `settings` WHERE `peer` = ?", [peer], async function (err, perm, f) {
        if (perm[0][cmdchk] > 0){
            connection.query("SELECT * FROM `admins` WHERE `peer` = ? AND `userid` = ? AND `status` >= ?", [peer, user, perm[0][cmdchk]], async function (err, checkwarnchk, f) {
                ...
                } else data.reply('Недостаточно прав для использования этой команды!')
            })
        } else {
            cmdsfnc.commands[cmdchk + 'command'](data)
        }
